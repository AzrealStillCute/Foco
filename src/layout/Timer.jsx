...
const Timer = ({
  ...
  document.onkeydown = (e) => {
    ...
    if (isKeyPressed["Control"] && isKeyPressed["Enter"])
      setCurrent(current === "study" ? "break" : "study");
  ...
  return (
    <div className="stopwatchContainer">
      <div className="stopwatch">
        <CircularProgress
          percentage={
            (((minute[current] - 1) * 60 + second) * 100) /
            (initialMinute[current] * 60)
          }
        />
        <img src={border} className="timerBorder" />
        <div className="timeContainer">
          <span className={`${active ? "minuteActive" : ""} minute`}>
            {minute[current]}
          </span>
          <span className={`${active ? "appear" : ""} second`}>
            {second < 10 ? "0" + second : second}
          </span>
        </div>
      </div>
      <div className="btnContainer">
        <button className="timerBtn" onClick={timeAdjustHandle}>
          <FontAwesomeIcon icon={faStopwatch} />
        </button>
        <button className="timerBtn" onClick={playPauseHandle}>
          <FontAwesomeIcon icon={pause ? faPlay : faPause} />
        </button>
        <button className="timerBtn" onClick={stopHandle}>
